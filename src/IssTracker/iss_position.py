...
from form_settings import SettingsForm
...
class IssPosition(tk.Tk):
    ...
    def can_i_see_iss(self) -> bool:
        """
        Check if it possible to see ISS Space Station
        :return:
        """
    ...
    def load_config(self):
        try:
            with open(CONFIG_FILE, 'r') as f:
                _json = json.load(f)
                if _json:
                    self.settings = SettingsModel.from_dict(_json)
    ...
    def loop(self):
        ...
        self.update_map()
        if self.can_i_see_iss() and not self.settings.email_me == "":
            print("sending mail")
            with smtplib.SMTP(self.settings.smtp_server) as connection:
                connection.starttls()
                connection.login(user=self.settings.email_me, password=self.settings.smtp_password)
                connection.sendmail(
                    from_addr=self.settings.email_me,
                    to_addrs=self.settings.email_me,
                    msg=f"Subject:ISS can bee seen now\n\nhttps://www.latlong.net/c/?lat={self.iss_latitude}&long={self.iss_longitude}"
                )
        self.after_id_loop = self.after(60000, func=self.loop)

    def menubar(self) -> tk.Menu:
        # image = self.read_first_image, compound = tk.LEFT
        menubar = tk.Menu(self)
        menu_file = tk.Menu(menubar, tearoff=0)
        menu_file.add_command(label='Settings', command=self.show_settings_form)
